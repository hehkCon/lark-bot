...
class PerformanceCommands:
    ...
    def _get_team_records(self, team_name: str, date_range: tuple) -> List[Dict]:
        ...
        print(f"DEBUG: Found {len(team_records)} records for {team_name}")
        return team_records
    ...
    def handle_performance_command(self, text: str, user_open_id: str) -> str:
        ...
        # Email-based user lookup
            ...
            if "[" in command_type and "](" in command_type:
                clean_email = command_type.split("[")[1].split("](")[0]
            else:
                clean_email = command_type.strip().lower()
